...
const RESUME_PHONE_FULL_QUERY = '(max-width: 767px), (max-width: 900px) and (max-height: 500px)'
...
export function ResumeScrollStory() {
  ...
  const phoneFullViewport = useMediaQuery(RESUME_PHONE_FULL_QUERY)
  const fullAnimationViewport = cinematicViewport || phoneFullViewport
  ...
  useEffect(() => {
    if (reducedMotion || !fullAnimationViewport) {
  ...
  }, [cinematicViewport, frames, fullAnimationViewport, phoneFullViewport, reducedMotion])
  ...
  useEffect(() => {
    ...
    if (!section || !sticky || !card || !paper || !ball || !shadow || !progressLine || !phase || reducedMotion || !fullAnimationViewport) return
  ...
  return (
    <section
      key={cinematicViewport ? 'resume-cinematic' : 'resume-phone-full'}
      ref={sectionRef}
      className={`resume-story${phoneFullViewport && !cinematicViewport ? ' phone-full-animation' : ''}`}
      aria-labelledby="resume-story-title"
    >
